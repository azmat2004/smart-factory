...
// Auto-initialize SQLite database - always ensures tables AND data exist
function ensureDatabase() {
  console.log('检查数据库状态...');

  // Step 1: Create all tables (idempotent - safe to run every time)
  db.exec(`
    CREATE TABLE IF NOT EXISTS t_factory (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, bak TEXT, factory_name TEXT, factory_img_url TEXT, factory_addr TEXT, factory_url TEXT, factory_worker INTEGER, factory_status INTEGER DEFAULT 0);
    CREATE TABLE IF NOT EXISTS t_user_role (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, role_desc TEXT, role_name TEXT, role_status INTEGER DEFAULT 0, factory_id INTEGER);
    CREATE TABLE IF NOT EXISTS t_user (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, user_status INTEGER DEFAULT 0, user_name TEXT NOT NULL, user_real_name TEXT, user_passwd TEXT NOT NULL, user_job_num TEXT, user_phone_num TEXT, user_email TEXT, role_id INTEGER, factory_id INTEGER NOT NULL);
    CREATE TABLE IF NOT EXISTS t_product (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, product_num TEXT NOT NULL, product_name TEXT NOT NULL, product_img_url TEXT, factory_id INTEGER NOT NULL);
    CREATE INDEX IF NOT EXISTS idx_pn ON t_product(product_num);
    CREATE TABLE IF NOT EXISTS t_equipment (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, equipment_seq TEXT NOT NULL, equipment_name TEXT, equipment_img_url TEXT, equipment_status INTEGER DEFAULT 10, factory_id INTEGER NOT NULL);
    CREATE TABLE IF NOT EXISTS t_equipment_product (id INTEGER PRIMARY KEY AUTOINCREMENT, equipment_id INTEGER NOT NULL, product_id INTEGER NOT NULL, yield INTEGER, unit INTEGER, factory_id INTEGER NOT NULL);
    ...
    CREATE INDEX IF NOT EXISTS idx_os ON t_product_order(order_seq);
    CREATE TABLE IF NOT EXISTS t_product_plan (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, plan_seq TEXT, order_id INTEGER NOT NULL, product_id INTEGER NOT NULL, plan_count INTEGER, delivery_date TEXT, plan_start_date TEXT, plan_end_date TEXT, plan_status INTEGER DEFAULT 10, factory_id INTEGER NOT NULL);
    CREATE INDEX IF NOT EXISTS idx_ps ON t_product_plan(plan_seq);
    ...
    CREATE TABLE IF NOT EXISTS t_order_track (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, schedule_id INTEGER, plan_id INTEGER, product_id INTEGER, working_count INTEGER DEFAULT 0, qualified_count INTEGER DEFAULT 0, factory_id INTEGER NOT NULL);
    CREATE INDEX IF NOT EXISTS idx_ot ON t_order_track(schedule_id);
    CREATE TABLE IF NOT EXISTS t_daily_work (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, schedule_id INTEGER NOT NULL, equipment_id INTEGER, equipment_seq TEXT, start_time TEXT, end_time TEXT, working_count INTEGER, qualified_count INTEGER, unqualified_cout INTEGER, complete_flag INTEGER DEFAULT 1, factory_id INTEGER NOT NULL, bak TEXT);
    CREATE TABLE IF NOT EXISTS t_permit (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, status INTEGER DEFAULT 0, parent_id INTEGER, permit_name TEXT, permit_desc TEXT, permit_path TEXT);
    CREATE TABLE IF NOT EXISTS t_role_permit (id INTEGER PRIMARY KEY AUTOINCREMENT, flag INTEGER DEFAULT 0, create_time TEXT, create_userid INTEGER, update_time TEXT, update_userid INTEGER, factory_id INTEGER, role_id INTEGER, permit_id INTEGER);
  `);

  // Step 2: Check if admin user exists, insert seed data if missing
  const userCheck = db.prepare('SELECT id FROM t_user WHERE user_name = ?').get('admin');
  if (userCheck) {
    console.log('数据库已就绪，管理员账户存在');
    return;
  }
  ...
  // Use individual INSERT OR IGNORE for safety
  const insertSqls = [
  ...
  console.log(`数据库初始化完成！插入了 ${inserted} 条数据。管理员: admin / 123456`);
}
